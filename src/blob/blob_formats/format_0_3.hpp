#ifndef GEOMETRY_BLOB_FORMAT_0_3_
#define GEOMETRY_BLOB_FORMAT_0_3_
...
namespace Geometry
{
    ...
    {
        struct format_0_3 : public format_base
        {
            typedef format_0_2 CAST_FROM;
            typedef std::array<float, 3> VEC3D;
            typedef std::array<float, 2> VEC2D;
            typedef std::array<uint32_t, 3> TRIANGLE;
            typedef std::pair< bool, std::vector< double > > GEOMETRY_DATA;
            ...
            typedef std::map< std::string, GEOMETRY_DATA > GEOMETRY_DATA_MAP;
            ...
            format_0_3& operator=( const CAST_FROM& other )
            {
                name = other.name;
                ...
                for( uint32_t p = 0; p < n_pieces; p++){
                    PIECE piece;
                    piece.name = other.pieces.at(p).name;
                    piece.vertices.resize( other.pieces.at(p).vertices.size() );
                    std::copy( other.pieces.at(p).vertices.begin(), other.pieces.at(p).vertices.end(), piece.vertices.begin() );
                    pieces.push_back( piece );
                }
                return *this;
            }

            virtual bool operator==( const format_0_3& other) const
            {
                bool is_equal = true;
                ...
                is_equal = is_equal && std::equal( geom_data.begin(), geom_data.end(), other.geom_data.begin());
            ...
            virtual bool self_check() const
            {
                ...
                for( auto it : geom_data ){
                    if( it.second.first && it.second.second.size() != n_faces ){
                        errout << "Self-Check failed! Geometry data '"<< it.first <<"' is non-zero in size but not equal to number of faces.";
                        throw BlobError::Consistency( errout.str() );
                    }
                    else if( !it.second.first && it.second.second.size() != n_vertices ){
                        errout << "Self-Check failed! Geometry data '"<< it.first <<"' is non-zero in size but not equal to number of vertices.";
                        throw BlobError::Consistency( errout.str() );
                    }
                }
            ...
            virtual void Load( std::istream& data )
            {
                    ...
                    // Read Curves
                {
                    ...
                    // Read Visual data flags
                uint32_t geom_data_count = 0x0;
                {
                    data.read( reinterpret_cast<char*>(&geom_data_count), sizeof(uint32_t ));
                }


                    // Skip reserved space
                {
                    uint32_t a; // Read 7 times
                    for( int i = 0; i < 7; i++ )
                        data.read( reinterpret_cast<char*>(&a), sizeof(uint32_t ) );
                }
                    ...
                    // Read Pieces (Vertices)
                {
                    ...
                    // Read Geometry Data
                {
                    for( uint32_t d = 0; d < geom_data_count; d++){
                        std::string name = read_string( data );
                        std::vector< double > data_array;
                        uint32_t is_face_centric;
                        data.read( reinterpret_cast<char*>( &is_face_centric ), sizeof( uint32_t ) );
                        if( is_face_centric ){
                            data_array.resize( n_faces );
                            data.read( reinterpret_cast<char*>( data_array.data() ), sizeof( double )*n_faces );
                        }
                        else{
                            data_array.resize( n_vertices );
                            data.read( reinterpret_cast<char*>( data_array.data() ), sizeof( double )*n_vertices );
                        }
                        auto res = geom_data.insert( {name, {is_face_centric>0, data_array}} );
                        if( !res.second ){
                            std::stringstream errout;
                            errout << "Error Loading Blob: Duplicate Geometry Data detected - '"<<name<<"' appears more than once.";
                            throw BlobError::Consistency( errout.str() );
                        }
                    }
                }
        ...
        virtual void Save( std::ostream& data ) const
            {
                ...
                {
                    uint32_t geom_data_count= static_cast<uint32_t>(geom_data.size());
                    data.write( reinterpret_cast<char*>( &geom_data_count ), sizeof( uint32_t ) );
                }

                    // Skip reserved space
                {
                    uint32_t a=0x0; // Write 7 times
                    for( int i = 0; i < 7; i++ )
                    ...
                    // Write Pieces (Vertices)
                {
                    ...
                    // Write Geometry Data
                {
                    for( const auto& it : geom_data ){

                        std::string name = it.first;
                        write_string( data, name );
                        std::vector< double > data_array;
                        uint32_t is_face_centric = it.second.first ? 0x1 : 0x0;
                        data.write( reinterpret_cast<const char*>( &is_face_centric ), sizeof( uint32_t ) );
                        if( is_face_centric ){
                            data.write( reinterpret_cast<const char*>( it.second.second.data() ), sizeof( double )*n_faces );
                        }
                        else{
                            data.write( reinterpret_cast<const char*>( it.second.second.data() ), sizeof( double )*n_vertices );
                        }
                    }
                }
...
#endif // GEOMETRY_BLOB_FORMAT_0_3_
